...
describe('StreamingTypeService', () => {
  ...
  describe('getStreamingTypeById', () => {
    ...
    it('should throw error when streaming type not found', async () => {
      ...
      await expect(streamingTypeService.getStreamingTypeById('invalid'))
        .rejects
        .toThrow(new StreamingServiceError('Streaming type not found', 404));
    });
  });
  ...
  describe('createStreamingType', () => {
    ...
    it('should throw error if name already exists', async () => {
      ...
      await expect(streamingTypeService.createStreamingType(newType))
        .rejects
        .toThrow(new StreamingServiceError('Streaming type name already exists', 400));
    });
    ...
    it('should throw error if name is missing', async () => {
      ...
      await expect(streamingTypeService.createStreamingType(invalidType))
        .rejects
        .toThrow(new StreamingServiceError('Name is required', 400));
    });
    ...
    it('should throw error if categories are missing', async () => {
      ...
      await expect(streamingTypeService.createStreamingType(invalidType))
        .rejects
        .toThrow(new StreamingServiceError('At least one category is required', 400));
    });
    ...
    it('should throw error if categories are empty', async () => {
      ...
      await expect(streamingTypeService.createStreamingType(invalidType))
        .rejects
        .toThrow(new StreamingServiceError('Invalid category data', 400));
    });

    it('should throw error if has two categories with the same id', async () => {
      const invalidType: Partial<IStreamingType> = {
        name: 'Netflix',
        categories: [{ id: 1, name: 'Movies' }, { id: 1, name: 'Series' }]
      };
      await expect(streamingTypeService.createStreamingType(invalidType))
        .rejects
        .toThrow(new StreamingServiceError('Duplicate category ID', 400));
    });
  });
  ...
  describe('addCategoryToStreamingType', () => {
    ...
    it('should throw error if streaming type not found', async () => {
      ...
      mockRepository.findById.mockResolvedValue(null);
      await expect(streamingTypeService.addCategoryToStreamingType('type1', newCategories))
        .rejects
        .toThrow(new StreamingServiceError('Streaming type not found', 404));
    });

    it('should throw error if categories already exist', async () => {
      const existingCategories: ICategory[] = [
        { id: 1, name: 'Movies' }
      ];
      mockRepository.findById.mockResolvedValue(mockStreamingType as IStreamingTypeResponse);
      await expect(streamingTypeService.addCategoryToStreamingType('type1', existingCategories))
        .rejects
        .toThrow(new StreamingServiceError('Categories already exist', 400));
    });
  });
  ...
  describe('updateStreamingType', () => {
    ...
    it('should throw error if streaming type not found', async () => {
      ...
      mockRepository.findById.mockResolvedValue(null);
      await expect(streamingTypeService.updateStreamingType('type1', updatedType))
        .rejects
        .toThrow(new StreamingServiceError('Streaming type not found', 404));
    });
  });
  ...
  describe('removeCategoryFromStreamingType', () => {
    it('should remove categories successfully', async () => {
      const categoriesToRemove: Partial<ICategory>[] = [
        { id: 1 }
      ];
      mockRepository.findById.mockResolvedValue(mockStreamingType as IStreamingTypeResponse);
    ...
    it('should throw error if categories not found in streaming type', async () => {
      const invalidCategories: Partial<ICategory>[] = [
        { id: 3 }
      ];
      mockRepository.findById.mockResolvedValue(mockStreamingType as IStreamingTypeResponse);

      await expect(streamingTypeService.removeCategoryFromStreamingType('type1', invalidCategories))
        .rejects
        .toThrow(new StreamingServiceError('Categories not found in streaming type: 3', 404));
    });
  });
  ...
  describe('deleteStreamingType', () => {
    ...
    it('should throw error if streaming type not found', async () => {
      ...
      await expect(streamingTypeService.deleteStreamingType('type1'))
        .rejects
        .toThrow(new StreamingServiceError('Streaming type not found', 404));
    });
  });
