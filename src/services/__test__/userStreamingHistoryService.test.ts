...
describe('UserStreamingHistoryService', () => {
  ...
  describe('getUserHistory', () => {
    ...
    it('should throw error when user history not found', async () => {
      ...
      await expect(service.getUserHistory(mockUserId))
        .rejects
        .toThrow(new StreamingServiceError('User history not found', 404));
    });
  });
  ...
  describe('addStreamingToHistory', () => {
    ...
    it('should throw error when streaming not found', async () => {
      ...
      await expect(service.addStreamingToHistory(mockUserId, mockStreamingEntry))
        .rejects
        .toThrow(new StreamingServiceError('Streaming not found', 404));
    });
    ...
    it('should throw error when invalid streaming title', async () => {
      ...
      await expect(service.addStreamingToHistory(mockUserId, mockStreamingEntry))
        .rejects
        .toThrow(new StreamingServiceError('Invalid streaming title', 400));
    });
    ...
    it('should throw error when streaming already exists in history', async () => {
      ...
      await expect(service.addStreamingToHistory(mockUserId, existingEntry))
        .rejects
        .toThrow(new StreamingServiceError('Streaming already in history', 400));
    });
  });
  ...
  describe('removeStreamingFromHistory', () => {
    it('should remove streaming from history successfully', async () => {
      ...
      expect(mockRepository.removeFromHistory).toHaveBeenCalledWith(
        mockUserId,
        mockStreamingId,
        mockDurationInMinutes
      );
    });
    ...
    it('should throw error when streaming not found in history', async () => {
      ...
      await expect(service.removeStreamingFromHistory(mockUserId, 'nonexistent'))
        .rejects
        .toThrow(new StreamingServiceError('Streaming not found in history', 404));
    });
    ...
    it('should throw error when user history failed to update', async () => {
      ...
      const mockStreamingId = mockHistory.watchHistory?.[0]?.streamingId || '';
      await expect(service.removeStreamingFromHistory(mockUserId, mockStreamingId))
        .rejects
        .toThrow(new StreamingServiceError('Failed to update history', 404));
    });
  });
  ...
  describe('validateStreamingData', () => {
    it('should throw error for missing streamingId', async () => {
      ...
      await expect(service.addStreamingToHistory(mockUserId, invalidData))
        .rejects
        .toThrow(new StreamingServiceError('Streaming ID is required', 400));
    });
    ...
    it('should throw error for missing title', async () => {
      ...
      await expect(service.addStreamingToHistory(mockUserId, invalidData))
        .rejects
        .toThrow(new StreamingServiceError('Title is required', 400));
    });
    ...
    it('should throw error for invalid duration', async () => {
      ...
      await expect(service.addStreamingToHistory(mockUserId, invalidData))
        .rejects
        .toThrow(new StreamingServiceError('Invalid duration', 400));
    });
  });
});